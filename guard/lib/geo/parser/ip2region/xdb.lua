...
local getShort,newBase,getLong
...
function _M:search(ip_src)
    ...
    local il0 = bit.band(bit.rshift(ip,24),0xFF)
    -- local il1 = (ip >> 16) & 0xFF
    local il1 = bit.band(bit.rshift(ip,16),0xFF)
    ...
    elseif content_buff ~= nil then
        ...
        -- load from the file
        local buff, err = read_data(self, HeaderInfoLength + idx, SegmentIndexSize)
        if err ~= nil then
...
function _M.check_ip(ip_str)
    ...
    for p in string.gmatch(ip_str..".", "([%d]+)%.") do
        ...
        ip = bit.bor(ip, bit.lshift(v, offset_arr[id]))
        id = id + 1
    end
...
function _M.long2ip(ip)
    ...
    return string.format("%d.%d.%d.%d", bit.band(bit.rshift(ip,24),0xFF), bit.band(bit.rshift(ip,16),0xFF), bit.band(bit.rshift(ip,8),0xFF), bit.band(ip,0xFF))
end
...
function getLong(buff, idx)
    ...
    local i2 = bit.lshift(string.byte(string.sub(buff, idx+1, idx+1)), 8)
    -- local i3 = (string.byte(string.sub(buff, idx+2, idx+2)) << 16)
    local i3 = bit.lshift(string.byte(string.sub(buff, idx+2, idx+2)),16)
    -- local i4 = (string.byte(string.sub(buff, idx+3, idx+3)) << 24)
    local i4 = bit.lshift(string.byte(string.sub(buff, idx+3, idx+3)),24)
    -- return (i1 | i2 | i3 | i4)
