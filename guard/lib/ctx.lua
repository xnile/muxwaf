...
local function decode_url_path(url)
  local decode_url = ngx_unescape_uri(url)
  local url_segment= net.url.parse(url):normalize()
  local path = url_segment.path and url_segment.path or ""
  if url_segment.path == "" then
    log.error(string_format("parse [%s] error", url))
    return url
  end
  return path
end
...
function _M.new()
  ...
  ctx.request_path = decode_url_path(ctx.var.uri)
