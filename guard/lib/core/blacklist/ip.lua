...
function _M.add(self, items)
  for _, item in ipairs(items) do
    ...
    if cache[id] then
      log.warn(string_format("faild add blacklist ip: the rule id '%s' already exists.", id))
      goto continue
    end

    local ok, err = tree:insert(ip, id)
    if not ok then
      log.error(string_format("faild add blacklist ip '%s': ", ip, err))
      goto continue
    end

    cache[id] = table_clone(item)
    log.debug(string_format("add blacklist ip '%s' success", ip))
...
function _M.del(self, items)
  ...
  for _, id in ipairs(items) do
    ...
    if not item then
      log.warn(string_format("faild delete blacklist ip '%s': the rule id '%s' does not exist", ip, id))
      goto continue
    end
    ...
    if not ok then
      log.error(string_format("faild delete blacklist ip '%s': %s", ip, err))
      goto continue
    end

    cache[id] = nil
    log.debug(string_format("delete blacklist ip '%s' success", ip))
...
function _M.full_sync(_, items)
  ...
  cache = new_cache
  tree = new_tree
  log.debug("full sync blacklist ip success")
end
