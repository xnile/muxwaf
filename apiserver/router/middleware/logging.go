...
func Logging() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		reg := regexp.MustCompile("(/v1/user|/auth/login)")
		...
		if err := json.Unmarshal(blw.body.Bytes(), &response); err != nil {
			logx.Errorf("response body can not unmarshal to model.Response struct, body: `%s`, err: %+v",
				blw.body.Bytes(), err)
		...
		logx.Infof("%-13s | %-12s | %s %s | {code: %d, message: %s}", latency, ip,
