...
func newZapLogger(cfg *Config) (Logger, error) {
	...
	// 设置初始化字段
	option := zap.Fields(zap.String("ip", utils.GetLocalIP()))
	...
	for _, w := range writers {
		...
		if w == WriterFile {
			logFilename := cfg.LoggerFile
			logWrite := getLogWriterWithTime(logFilename)
			...
			core := zapcore.NewCore(encoder, zapcore.AddSync(logWrite), level)
			cores = append(cores, core)
		}
