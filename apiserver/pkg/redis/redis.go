...
func NewRedis() (*Redis, error) {
	var address = viper.GetString("redis.host")
	var port = viper.GetString("redis.port")
	var database = viper.GetString("redis.db")
...
func (r *Redis) Set(key string, value interface{}, ttl time.Duration) error {
	...
	ttlInt := int64(ttl)
	if ttlInt != 0 {
		_, err = c.Do("SET", key, value, "EX", ttlInt)
	} else {
